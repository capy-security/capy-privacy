...
function _page($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    let currentPage = 1;
    let itemsPerPage = 12;
    const tableQuery = createReadObjectDnsTableGet(() => Tables.category, () => ({ page_number: currentPage, items_per_page: itemsPerPage }), () => ({
    ...
    let totalPages = derived(() => Math.max(1, Math.ceil(totalItems() / itemsPerPage) || (categories().length > 0 ? 1 : 0)));
    ...
    $$renderer2.push(`<!----> <span class="hidden md:inline">Create Category</span></button></div></div> `);
    if (error()) {
      $$renderer2.push("<!--[0-->");
    ...
    if (isLoading() && categories().length === 0) {
      ...
      $$renderer2.push("<!--[1-->");
      $$renderer2.push(`<div class="rounded-lg border border-white/20 bg-white/10 p-8 text-center backdrop-blur-sm"><p class="text-gray-300">No categories found</p></div>`);
    } else {
      $$renderer2.push("<!--[-1-->");
      ...
      for (let $$index_2 = 0, $$length = each_array_1.length; $$index_2 < $$length; $$index_2++) {
        ...
        $$renderer2.push(`<div class="relative rounded-lg border border-white/20 bg-white/10 p-6 shadow-lg backdrop-blur-sm transition-all hover:bg-white/15"><div class="absolute top-2 right-2 flex gap-1"><button class="rounded-md p-1.5 text-gray-300 transition-colors hover:bg-white/20 hover:text-white" aria-label="Edit category">`);
        PencilSquare($$renderer2, { class: "h-5 w-5" });
        $$renderer2.push(`<!----></button> <button class="rounded-md p-1.5 text-gray-300 transition-colors hover:bg-red-500/20 hover:text-red-300" aria-label="Delete category">`);
        Trash($$renderer2, { class: "h-5 w-5" });
        $$renderer2.push(`<!----></button></div> <div class="space-y-2">`);
        if (category.id !== void 0) {
          $$renderer2.push("<!--[0-->");
          $$renderer2.push(`<div class="text-xs text-gray-400">ID: ${escape_html(category.id)}</div>`);
        } else {
          $$renderer2.push("<!--[-1-->");
        }
        $$renderer2.push(`<!--]--> `);
        if (category.name !== void 0) {
          $$renderer2.push("<!--[0-->");
          $$renderer2.push(`<h3 class="text-xl font-semibold text-white">${escape_html(category.name)}</h3>`);
        } else {
          $$renderer2.push("<!--[-1-->");
        }
        $$renderer2.push(`<!--]--> `);
        if (category.description !== void 0) {
          $$renderer2.push("<!--[0-->");
          $$renderer2.push(`<p class="text-sm text-gray-400">${escape_html(category.description)}</p>`);
        } else {
          $$renderer2.push("<!--[-1-->");
        }
        $$renderer2.push(`<!--]--> <!--[-->`);
        const each_array_2 = ensure_array_like(Object.entries(category));
        for (let $$index_1 = 0, $$length2 = each_array_2.length; $$index_1 < $$length2; $$index_1++) {
          let [key, value] = each_array_2[$$index_1];
          if (!["id", "name", "description"].includes(key) && value !== null && value !== void 0) {
            $$renderer2.push("<!--[0-->");
    ...
    if (isEditModalOpen && editingCategory) {
      ...
      $$renderer2.push(`<!--]--> <div><label for="edit-category-name" class="block text-sm font-medium text-gray-200">Name <span class="text-red-400">*</span></label> <input id="edit-category-name" type="text" required=""${attr("value", editFormData.name)} placeholder="Category name" class="mt-1 block w-full rounded-md border border-white/20 bg-white/10 px-3 py-2 text-white placeholder-gray-400 backdrop-blur-sm focus:border-violet-500 focus:ring-1 focus:ring-violet-500 focus:outline-none"${attr("disabled", isUpdating(), true)}/></div> <div><label for="edit-category-description" class="block text-sm font-medium text-gray-200">Description</label> <textarea id="edit-category-description" placeholder="Category description (optional)" rows="3" class="mt-1 block w-full rounded-md border border-white/20 bg-white/10 px-3 py-2 text-white placeholder-gray-400 backdrop-blur-sm focus:border-violet-500 focus:ring-1 focus:ring-violet-500 focus:outline-none"${attr("disabled", isUpdating(), true)}>`);
      const $$body = escape_html(editFormData.description);
      if ($$body) {
        $$renderer2.push(`${$$body}`);
      }
      ...
      PencilSquare($$renderer2, { class: "h-5 w-5" });
      $$renderer2.push(`<!----> <span class="hidden md:inline">${escape_html(isUpdating() ? "Updating..." : "Update Category")}</span></button></div></form></div>`);
    } else {
      $$renderer2.push("<!--[-1-->");
    }
    $$renderer2.push(`<!--]--> `);
    if (isModalOpen) {
      $$renderer2.push("<!--[0-->");
      $$renderer2.push(`<div class="fixed inset-0 z-[2000] bg-black/50 backdrop-blur-sm" role="button" tabindex="0" aria-label="Close modal"></div> <div class="fixed top-1/2 left-1/2 z-[2001] w-full max-w-md -translate-x-1/2 -translate-y-1/2 transform rounded-lg border border-white/20 bg-gradient-to-br from-violet-900/90 to-purple-900/90 p-6 shadow-2xl backdrop-blur-md" role="dialog" aria-modal="true" aria-labelledby="modal-title" tabindex="-1"><div class="mb-4 flex items-center justify-between"><h2 id="modal-title" class="text-2xl font-bold text-white">Create New Category</h2> <button class="rounded-md p-1 text-gray-300 transition-colors hover:bg-white/10 hover:text-white" aria-label="Close modal">`);
      XMark($$renderer2, { class: "h-6 w-6" });
      $$renderer2.push(`<!----></button></div> <form class="space-y-4">`);
      if (createError) {
        $$renderer2.push("<!--[0-->");
      ...
      $$renderer2.push(`<!--]--> <div><label for="category-name" class="block text-sm font-medium text-gray-200">Name <span class="text-red-400">*</span></label> <input id="category-name" type="text" required=""${attr("value", formData.name)} placeholder="Category name" class="mt-1 block w-full rounded-md border border-white/20 bg-white/10 px-3 py-2 text-white placeholder-gray-400 backdrop-blur-sm focus:border-violet-500 focus:ring-1 focus:ring-violet-500 focus:outline-none"${attr("disabled", isCreating(), true)}/></div> <div><label for="category-description" class="block text-sm font-medium text-gray-200">Description</label> <textarea id="category-description" placeholder="Category description (optional)" rows="3" class="mt-1 block w-full rounded-md border border-white/20 bg-white/10 px-3 py-2 text-white placeholder-gray-400 backdrop-blur-sm focus:border-violet-500 focus:ring-1 focus:ring-violet-500 focus:outline-none"${attr("disabled", isCreating(), true)}>`);
      const $$body_1 = escape_html(formData.description);
      if ($$body_1) {
      ...
      Plus($$renderer2, { class: "h-5 w-5" });
      $$renderer2.push(`<!----> <span class="hidden md:inline">${escape_html(isCreating() ? "Creating..." : "Create Category")}</span></button></div></form></div>`);
    } else {
      $$renderer2.push("<!--[-1-->");
    }
    $$renderer2.push(`<!--]--> `);
    if (isDeleteModalOpen && categoryToDelete) {
      $$renderer2.push("<!--[0-->");
      $$renderer2.push(`<div class="fixed inset-0 z-[2000] bg-black/50 backdrop-blur-sm" role="button" tabindex="0" aria-label="Close modal"></div> <div class="fixed top-1/2 left-1/2 z-[2001] w-full max-w-md -translate-x-1/2 -translate-y-1/2 transform rounded-lg border border-white/20 bg-gradient-to-br from-violet-900/90 to-purple-900/90 p-6 shadow-2xl backdrop-blur-md" role="dialog" aria-modal="true" aria-labelledby="delete-modal-title" tabindex="-1"><div class="mb-4 flex items-center justify-between"><h2 id="delete-modal-title" class="text-2xl font-bold text-white">Delete Category</h2> <button class="rounded-md p-1 text-gray-300 transition-colors hover:bg-white/10 hover:text-white" aria-label="Close modal">`);
      XMark($$renderer2, { class: "h-6 w-6" });
      ...
      $$renderer2.push(`<!--]--> <p class="text-gray-200">Are you sure you want to delete the category <span class="font-semibold text-white">${escape_html(categoryToDelete.name || `ID: ${categoryToDelete.id}`)}</span> ? This action cannot be undone.</p> <div class="flex justify-end gap-3 pt-4"><button type="button"${attr("disabled", isDeleting(), true)} class="flex items-center justify-center p-2 md:px-4 md:py-2 rounded-md border border-white/20 bg-white/10 font-medium text-white transition-colors hover:bg-white/20 disabled:cursor-not-allowed disabled:opacity-50"><span class="hidden md:inline">Cancel</span> <span class="md:hidden">✕</span></button> <button type="button"${attr("disabled", isDeleting(), true)} class="flex items-center justify-center p-2 md:px-4 md:py-2 md:gap-2 rounded-md bg-gradient-to-r from-red-600 to-red-700 font-medium text-white shadow-lg transition-all hover:from-red-700 hover:to-red-800 disabled:cursor-not-allowed disabled:opacity-50">`);
      Trash($$renderer2, { class: "h-5 w-5" });
      $$renderer2.push(`<!----> <span class="hidden md:inline">${escape_html(isDeleting() ? "Deleting..." : "Delete Category")}</span></button></div></div></div>`);
    } else {
      $$renderer2.push("<!--[-1-->");
    }
    $$renderer2.push(`<!--]-->`);
